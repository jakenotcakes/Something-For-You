document.addEventListener("DOMContentLoaded", function () {
  ...
  // Smooth scroll for in-page links for change
  ...
  if (
    countdown &&
    (countdown.dataset.date || countdown.dataset.mode === "clock")
  ) {
    const targetDate = countdown.dataset.date
      ? new Date(countdown.dataset.date)
      : null;
    const isClock = countdown.dataset.mode === "clock";
    ...
    function updateCountdown() {
      ...
      if (today >= targetDay) {
        ...
        countdownLock = false;
        clickLock = false;
        revealHint.textContent = "Click to reveal the content!❤️🖤";
        if (mainContent) {
          mainContent.classList.add("hidden");
        }
        if (countdownCover) {
          countdownCover.classList.remove("revealed");
        }
        return;
      }
      ...
      if (diff <= 0 || typeof countdownLock === "undefined" || !countdownLock) {
        ...
        countdownLock = false;
        clickLock = false;
        revealHint.textContent = "Time's up! Click to enter the page.";
        if (mainContent) {
          mainContent.classList.add("hidden");
        }
        if (countdownCover) {
          countdownCover.classList.remove("hidden");
        }
        return;
      }
      let remaining = diff;
